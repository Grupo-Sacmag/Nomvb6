...
End Type
 ...
 Public Function EsEmpresaLocal(ByVal nombreEmpresa As String) As Boolean
    ' Empresas que NO deben tocar la BD remota bajo ninguna circunstancia
    EsEmpresaLocal = (InStr(1, UCase(Trim(nombreEmpresa)), "JRI") > 0)
End Function
...
Sub calculo(base, impto, psub)
    ...
    If Form8.Option4.Value = True Then 'Retencion mensual
        ...
        A�oNomina = Right(Trim(Form8.Label7.Caption), 4)
        Close 35
        Open CStr(Trim(Form1.Dir1)) + "\" + mesNomina + "1" + A�oNomina + ".NOM" For Random As 35 Len = Len(nomina)
        ...
        Close 35: Close 36
        Open CStr(Trim(Form1.Dir1)) + "\" + mesNomina + "1" + A�oNomina + ".cmp" For Random As 36 Len = Len(nom_com)
